...
import {
  useConservationLogs,
  useCreateConservationLog,
} from '../../hooks/useConservation';
...
export default function ConditionHistoryTimeline({ artifact }) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const artifactId = artifact?.id;

  const { data, isLoading, isError } = useConservationLogs(
    { artifact_id: artifactId, limit: 50 },
    { enabled: Boolean(artifactId) }
  );
  const createMutation = useCreateConservationLog();
  const logs = data?.conservation_logs || [];
  ...
  const handleSaveLog = async (e) => {
    ...
    if (!artifactId) {
      toast.error('Artifact is required to log an inspection');
      return;
    }
    ...
    try {
      ...
      toast.error(getApiErrorMessage(error, 'Failed to save conservation log'));
    }
  };
  ...
  return (
    <Card hover>
      ...
      {!isLoading && isError && (
        <p className="text-xs text-[#8B1E1E] py-4">
          Unable to load conservation logs. You may need a conservation, curator, or admin role.
        </p>
      )}

      {!isLoading && !isError && logs.length === 0 && (
        <p className="text-xs text-[#6E5445] py-4">
          No conservation inspections logged yet for this artifact.
        </p>
      )}
      ...
      {!isLoading && !isError && logs.length > 0 && (
        <div className="relative space-y-6 pl-4 before:absolute before:left-2 before:top-2 before:bottom-2 before:w-0.5 before:bg-[#E2D6C5]">
          {logs.map((log) => {
            const inspectorName = log.inspector?.name || 'Conservation Specialist';
            return (
                ...
                <div className="rounded-xl border border-[#E2D6C5] bg-[#FFFDF9] p-4 shadow-2xs space-y-2.5">
                  <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#F0E6D8] pb-2 text-xs">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {log.condition_before && <Badge variant={log.condition_before}>{log.condition_before}</Badge>}
                      {log.condition_before && log.condition_after && (
                        <span className="text-xs text-[#A08878]">→</span>
                      )}
                      {log.condition_after && <Badge variant={log.condition_after}>{log.condition_after}</Badge>}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 pt-1 text-[11px] text-[#6E5445]">
                    {log.next_inspection_date && (
                      <span>
                        Next Inspection Due:{' '}
                        <strong className="text-[#2B1B12]">{log.next_inspection_date}</strong>
                      </span>
                    )}
